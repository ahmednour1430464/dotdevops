...
| v0.5 | Nested steps, For-loops | ✅ Stable |
| v0.6 | Step parameters | ✅ Stable |
| v0.7 | Step libraries (imports) | ✅ Stable |
| v0.8 | Fleets, Node Contracts, Target Labels | ✅ Stable |
| v0.9 | Secret injection model | 🔧 Planned |
| v1.0 | LSP, plan diff, structured output | 🔧 Planned |

**Default version**: auto-detected from the `version` directive in each plan file.

Specify version explicitly with `--lang` flag (deprecated — use the `version` directive instead):
```bash
...
All agent communication should be secured with mutual TLS (mTLS) to prevent unauthorized access. The agent will accept connections from any caller without mTLS if not configured — **do not expose the agent on untrusted networks without mTLS**.

### Quick Certificate Setup

Generate development certificates with the built-in PKI tool:

```bash
devopsctl pki init --out ./pki
```

This creates six files in `./pki/`:
- `ca.crt` / `ca.key` — self-signed certificate authority
- `controller.crt` / `controller.key` — certificate for the controller (CLI)
- `agent.crt` / `agent.key` — certificate for the agent daemon

### Starting the Agent with mTLS

```bash
devopsctl agent \
  --addr :7700 \
  --contexts /etc/devopsctl/contexts.yaml \
  --tls-cert ./pki/agent.crt \
  --tls-key  ./pki/agent.key \
  --tls-ca   ./pki/ca.crt
```

### Running Commands with mTLS

All controller commands accept the same three flags:

```bash
devopsctl apply plan.devops \
  --tls-cert ./pki/controller.crt \
  --tls-key  ./pki/controller.key \
  --tls-ca   ./pki/ca.crt

devopsctl reconcile plan.devops --tls-cert ./pki/controller.crt --tls-key ./pki/controller.key --tls-ca ./pki/ca.crt
devopsctl observe  plan.devops --tls-cert ./pki/controller.crt --tls-key ./pki/controller.key --tls-ca ./pki/ca.crt
devopsctl rollback --last      --tls-cert ./pki/controller.crt --tls-key ./pki/controller.key --tls-ca ./pki/ca.crt
```

> For production, use certificates from an external CA (e.g., HashiCorp Vault PKI, AWS Private CA). The `pki init` tool is for development and homelab use only.
...
```
devopsctl/
├── .qoder/                 # Architectural audit documents for contributors (see Development Tools below)
...
│   ├── v0_3/ … v0_8/       # Per-version valid/invalid/hash-stability cases
├── BUILTINS.md             # Irreducible built-in reference (v1.1+)
├── DESIGN.md               # Architecture principles and invariants
...
The `.qoder/` directory contains architectural audit documents generated by code-analysis tooling for contributors. These documents (like `ARCHITECTURAL_AUDIT.md`) describe invariant verification, design analysis, and implementation guidance. They are committed to the repository intentionally — they are part of the project's engineering record, not IDE-specific state.

References to `.qoder/` documents appear in `LANGUAGE_VERSIONS.md` under "References".

### Contributing

Contributions are welcome! Please:
1. Read [DESIGN.md](DESIGN.md) — understand the four invariants before writing any language code
2. Read [LANGUAGE_VERSIONS.md](LANGUAGE_VERSIONS.md) — understand which features belong to which version
3. Add tests: every new language feature needs valid, invalid, and hash-stability test cases
4. Never ship a language change without updating the LSP so editors never see unknown syntax
